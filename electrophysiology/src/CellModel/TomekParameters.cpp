...
TomekParameters::TomekParameters(const char *initFile, ML_CalcType tinc) {
  P = new Parameter[vtLast];
  Init(initFile, tinc);
}

TomekParameters::~TomekParameters() {}

void TomekParameters::PrintParameters() {
  cout << "TomekParameters:"<<endl;
  for (int i = vtFirst; i < vtLast; i++) {
    cout << "\t" << P[i].name << "\t= "     << P[i].value <<endl;
  }
}

void TomekParameters::Init(const char *initFile, ML_CalcType tinc) {
#if KADEBUG
  cerr << "Loading the Tomek parameter from " << initFile << " ...\n";
#endif  // if KADEBUG
  ...
  ParameterLoader EPL(initFile, EMT_Tomek);
  ...
  cerr << "#Init() done ... \n";
#endif  // if KADEBUG
}  // TomekParameters::Init

void TomekParameters::Calculate() {
#if KADEBUG
  cerr << "#TomekParameters - Calculate ..." << endl;
#endif  // if KADEBUG
...
}  // TomekParameters::Calculate

void TomekParameters::InitTable(ML_CalcType tinc) {
#if KADEBUG
  cerr << "#TomekParameters - InitTable ..." << endl;
#endif  // if KADEBUG
