...
inline bool OHaraRudy::AddHeteroValue(string desc, double val) {
...
inline unsigned char OHaraRudy::getSpeed(ML_CalcType adVm) {
  return (unsigned char)5;
}

void OHaraRudy::Init() {
#if KADEBUG
  cerr << "#initializing Class: OHaraRudy ... " << endl;
        #endif  // if KADEBUG
...
ML_CalcType OHaraRudy::Calc(double tinc,  ML_CalcType V,  ML_CalcType i_external,  ML_CalcType stretch, int euler) {
...
#ifdef DUTTA
  const ML_CalcType tau_h_fast = 1.0 / (3.686e-6 * exp(-(V_m + 3.8875)/7.8579) + 16.0 * exp((V_m - 0.4963)/9.1843));  //
                                                                                                                      //
                                                                                                                      // Dutta
                                                                                                                      //
                                                                                                                      // et
...
#ifdef DUTTA
  const ML_CalcType tau_j = 4.8590 + 1.0 /
    (0.8628 * exp(-(V_m + 116.7258)/7.6005) + 1.1096 * exp((V_m + 6.2719)/9.0358));  // Dutta 2017
#else  // ifdef DUTTA
  const ML_CalcType tau_j = 2.038 + 1.0 / (0.02136 * exp(-(V_m + 100.6)/8.281) + 0.3052 * exp((V_m + 0.9941)/38.45));  //
                                                                                                                       //
                                                                                                                       // ORd
#endif  // ifdef DUTTA
  ...
  I_Na_fast = v(VT_INaF_Multiplier) * G_Na_fast * (V_m - E_Na) *m*m*m*
    ((1.0 - phi_INa_CaMK) * h * j + phi_INa_CaMK * h_CaMK * j_CaMK);
  ...
  const ML_CalcType Psi_Ca   = z_Ca * z_Ca * VFFoverRT*
    (gamma_Cai * Ca_ss * exp_z_Ca - gamma_Cao * v(VT_Ca_o)) / (exp_z_Ca - 1.0);
  const ML_CalcType Psi_CaNa = z_Na * z_Na * VFFoverRT *
    (gamma_Nai * Na_ss * exp_z_Na - gamma_Nao * v(VT_Na_o)) / (exp_z_Na - 1.0);
  const ML_CalcType Psi_CaK = z_K * z_K * VFFoverRT *
    (gamma_Ki * K_ss * exp_z_K - gamma_Ko * v(VT_K_o)) / (exp_z_K - 1.0);
  const ML_CalcType I_CaL_b       = v(VT_P_Ca) * Psi_Ca;
  const ML_CalcType I_CaNa_b      = v(VT_P_CaNa) * Psi_CaNa;
  const ML_CalcType I_CaK_b       = v(VT_P_CaK) * Psi_CaK;
  const ML_CalcType phi_ICaL_CaMK = phi_INa_CaMK;
  const ML_CalcType I_CaL_CaMK    = v(VT_P_Ca_CaMK) * Psi_Ca;
  const ML_CalcType I_CaNa_CaMK   = v(VT_P_CaNa_CaMK) * Psi_CaNa;
  const ML_CalcType I_CaK_CaMK    = v(VT_P_CaK_CaMK) * Psi_CaK;
  I_CaL = I_CaL_b * d * (1.0 - phi_ICaL_CaMK) * (f * (1.0 - n) + f_Ca * n * j_Ca) +
    I_CaL_CaMK * d *
    phi_ICaL_CaMK * (f_CaMK * (1.0 - n) + f_Ca_CaMK * n * j_Ca);
  I_CaNa = I_CaNa_b * d * (1.0 - phi_ICaL_CaMK) * (f * (1.0 - n) + f_Ca * n * j_Ca) + I_CaNa_CaMK *
    d * phi_ICaL_CaMK * (f_CaMK * (1.0 - n) + f_Ca_CaMK * n * j_Ca);
  I_CaK = I_CaK_b * d * (1.0 - phi_ICaL_CaMK) * (f * (1.0 - n) + f_Ca * n * j_Ca) + I_CaK_CaMK * d *
    phi_ICaL_CaMK * (f_CaMK * (1.0 - n) + f_Ca_CaMK * n * j_Ca);
  ...
  const ML_CalcType J_rel        = (1.0 - phi_rel_CaMK) * J_rel_NP + phi_rel_CaMK * J_rel_CaMK;
  ...
  const ML_CalcType J_up =
    ...
    v(VT_HF_Gomez_0) *
#endif // ifdef HF
    ((1.0 - phi_up_CaMK) * J_up_NP + phi_up_CaMK * J_up_CaMK - J_leak);
...
void OHaraRudy::Print(ostream &tempstr, double tArg,  ML_CalcType V) {
  tempstr << tArg << ' ' << V << ' ' <<
    m << ' '  << h_fast << ' ' << h_slow << ' ' << j << ' ' << h_CaMK_slow << ' ' << j_CaMK << ' ' << m_L << ' ' <<
    h_L << ' ' << h_L_CaMK << ' ' << a << ' ' <<
    i_fast << ' ' << i_slow << ' ' << a_CaMK << ' ' << i_CaMK_fast << ' ' << i_CaMK_slow << ' ' << d << ' ' << f_fast <<
    ' ' << f_slow << ' ' << f_Ca_fast << ' ' << f_Ca_slow << ' ' <<
    j_Ca << ' ' << f_CaMK_fast << ' ' << f_Ca_CaMK_fast << ' ' << n << ' ' << x_r_fast << ' ' << x_r_slow << ' ' <<
    x_s1 << ' ' << x_s2 << ' ' << x_K1 << ' ' <<
    Na_i << ' ' << Na_ss << ' ' << K_i << ' ' << K_ss << ' ' << Ca_i << ' ' << Ca_ss << ' ' << Ca_nsr << ' ' <<
    Ca_jsr << ' ' << CaMK_trap << ' ' << J_rel_NP << ' ' << J_rel_CaMK << ' '
    #ifdef TRPN
...
void OHaraRudy::LongPrint(ostream &tempstr, double tArg,  ML_CalcType V) {
...
void OHaraRudy::GetParameterNames(vector<string> &getpara) {
  const string ParaNames[] =
  {
    ...
    "f_Ca_fast",
    "f_Ca_slow",
    "j_Ca",        "f_CaMK_fast",        "f_Ca_CaMK_fast",          "n",                            "x_r_fast",
    "x_r_slow",
    "x_s1",
    "x_s2",        "x_K1",               "Na_i",                    "Na_ss",                        "K_i",
    "K_ss",
    "Ca_i",        "Ca_ss",
...
}  // OHaraRudy::GetParameterNames

void OHaraRudy::GetLongParameterNames(vector<string> &getpara) {
