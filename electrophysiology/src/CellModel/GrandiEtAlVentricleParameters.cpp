/* File: GrandiEtAlVentricleParameters.cpp
...
GrandiEtAlVentricleParameters::GrandiEtAlVentricleParameters(const char *initFile, ML_CalcType tinc) {
  P = new Parameter[vtLast];
  Init(initFile, tinc);
}

GrandiEtAlVentricleParameters::~GrandiEtAlVentricleParameters() {}

void GrandiEtAlVentricleParameters::PrintParameters() {
  cout << "GrandiEtAlVentricleParameters:"<<endl;
  for (int i = vtFirst; i < vtLast; i++) {
    cout << "\t" << P[i].name << "\t= "     << P[i].value <<endl;
  }
}

void GrandiEtAlVentricleParameters::Init(const char *initFile, ML_CalcType tinc) {
#if KADEBUG
  cerr << "Loading the GrandiEtAlVentricle parameter from " << initFile << " ...\n";
#endif  // if KADEBUG
  ...
  P[VT_x_to_s_init].name    = "x_to_s_init";
  P[VT_y_to_s_init].name    = "y_to_s_init";
  ...
  P[VT_GtoSlow].readFromFile       = false;
  P[VT_GtoFast].readFromFile       = false;
  ...
  ParameterLoader EPL(initFile, EMT_GrandiEtAlVentricle);
  ...
  cerr << "#Init() done ... \n";
#endif  // if KADEBUG
}  // GrandiEtAlVentricleParameters::Init

void GrandiEtAlVentricleParameters::Calculate() {
#if KADEBUG
  cerr << "#GrandiEtAlVentricleParameters - Calculate ..." << endl;
#endif  // if KADEBUG
  ...
  P[VT_gkr].value     =  0.0350000* pow(((P[VT_K_o].value)/5.40000), 1.0 / 2);
  P[VT_GtoSlow].value = ((P[VT_epi].value) == 1.00000 ?  1.00000*0.130000*0.120000 :  0.130000*0.300000*0.964000);
  P[VT_GtoFast].value =
    ((P[VT_epi].value) == 1.00000 ?  1.00000*0.130000*0.880000 :  0.130000*0.300000*0.0360000);
  ...
  P[VT_Ikiconst].value      = 0.35* sqrt(P[VT_K_o].value/5.4);
}  // GrandiEtAlVentricleParameters::Calculate

void GrandiEtAlVentricleParameters::InitTable(ML_CalcType tinc) {
#if KADEBUG
  cerr << "#GrandiEtAlVentricleParameters - InitTable ..." << endl;
#endif  // if KADEBUG
  ...
  for (double V = -RangeTabhalf+.0001; V < RangeTabhalf; V += dDivisionTab) {
    ...
    xsss[Vi]       = 1.00000/(1.00000+(exp((-(V+3.80000)/14.2500))));
    exptauxs[Vi]   =  exp(-HT/ (990.100/(1.00000+(exp((-(V+2.43600)/14.1200))))));
    xtoss[Vi]      = 1.00000/(1.00000+(exp((-(V - 19.0000)/13.0000))));
    exptauxtos[Vi] =  exp(-HT/ (9.00000/(1.00000+(exp(((V+3.00000)/15.0000))))+0.500000));
    ytoss[Vi]      = 1.00000/(1.00000+(exp(((V+19.5000)/5.00000))));
    exptauytos[Vi] =  exp(-HT/ (800.000/(1.00000+(exp(((V+60.0000)/10.0000))))+30.0000));
    exptauxtof[Vi] =  exp(-HT/ (8.50000*(exp((-(pow(((V+45.0000)/50.0000), 2.00000)))))+0.500000));
    exptauytof[Vi] =  exp(-HT/ (85.0000*(exp((-(pow((V+40.0000), 2.00000))/220.000)))+7.00000));
    dss[Vi]        = 1.00000/(1.00000+(exp((-(V+5.00000)/6.00000))));
    exptaud[Vi]    = exp(-HT/ ((1.00000*dss[Vi]*(1.00000 - (exp((-(V+5.00000)/6.00000)))))/(0.0350000*(V+5.00000))));
    fss[Vi]        = 1.00000/(1.00000+(exp(((V+35.0000)/9.00000))))+0.600000/(1.00000+(exp(((50.0000 - V)/20.0000))));
    exptauf[Vi]    =  exp(-HT/ (1.00000/(0.0197000*(exp((-(pow((0.0337000*(V+14.5000)), 2.00000)))))+0.0200000)));
...
}  // GrandiEtAlVentricleParameters::InitTable
